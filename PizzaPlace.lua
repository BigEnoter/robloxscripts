	...
	if doSupplier then
		...
		if refill then
			...
			while doSupplier do
				...
				for s,c in pairs(supplyCounts) do
					if c<settings.refill_end then
						...
						if #boxes > 30 then
							for i=1,#boxes do
								local box = boxes[i]
								if bcolorToSupply[box.BrickColor.Name]==s and box.Anchored==false and box.Position.Z < -740 then
									count=count+1
								end
							end
						end
				...
				if not waiting then
					--move boxes
					root.CFrame=CFrame.new(8,12.4,-795)
					wait(0.1)
					alt=1-alt
					lastBox=nil
					local j=0
					...
					for i=1,#boxes do
						local box = boxes[i]
						if box.Anchored==false and box.Position.Z < -740 and bcolorToSupply[box.BrickColor.Name] and supplyCounts[bcolorToSupply[box.BrickColor.Name]]<settings.refill_end then
